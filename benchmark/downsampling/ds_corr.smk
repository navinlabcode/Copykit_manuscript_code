...
rule all:
    input:
        ...
        expand('downsampled_sort_25k/{sample}.bam', sample=samples),
        expand('downsampled_sort_10k/{sample}.bam', sample=samples)
...
rule downsampling_25k:
    input:
        "marked/{sample}.bam"
    output:
        temp("temp_downsampled_25k/{sample}.sam")
    params:
        reads=25000
    shell:
        "{samtools_path} view {input} | shuf -n {params.reads} --random-source={input} >> {output}"
...
rule add_header_25k:
    input: 
        header = 'temp_header/{sample}.sam',
        reads = 'temp_downsampled_25k/{sample}.sam'
    output:
        temp('downsampled_25k/{sample}.sam')
    shell:
        "cat {input.header} {input.reads} > {output}"

rule sort_25k:
    input: 
        "downsampled_25k/{sample}.sam"
    output:
...
rule downsampling_10k:
    input:
        "marked/{sample}.bam"
    output:
        temp("temp_downsampled_10k/{sample}.sam")
    params:
        reads=10000
    shell:
        "{samtools_path} view {input} | shuf -n {params.reads} --random-source={input} >> {output}"
...
rule add_header_10k:
    input: 
        header = 'temp_header/{sample}.sam',
        reads = 'temp_downsampled_10k/{sample}.sam'
    output:
        temp('downsampled_10k/{sample}.sam')
    shell:
        "cat {input.header} {input.reads} > {output}"

rule sort_10k:
    input: 
        "downsampled_10k/{sample}.sam"
    output:
